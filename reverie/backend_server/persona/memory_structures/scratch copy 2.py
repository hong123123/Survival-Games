...
from utils import HEAL, HP_MAX, FULLNESS_MAX, INIT_FOOD
...
class Scratch:
    ...
    """
        ...
        # hong add life
        d = {k:{kk:vv for kk,vv in v.items() if kk!='instance'} for k,v in self.global_aware.items() if k != self.name}
        commonset += f"""Hp: {self.hp}
Fullness: {self.fullness}
Food: {self.food}
Global vitality information: {d}

Current situation:
You are trapped in a isolated town, you cant get any outside supplies until the rescue come. You have limitted resources, and so do other people.
You should do whatever you can to avoid death.

Life cycle rule:
1) Per eating, fullness +1 and food -1. Cannot eat more than 1 food each meal, fullness would not increase if reach the maximum of 3). If really eating, should say it clearly. Only say it when you are really doing it.
2) At the end of the day, if fullness reaches maximum, hp would get +1 (healing effect)(, with maximun hp of 6); else if fullness does not reach maximum, hp would minus the gap to maximum (starving effect) (e.g. at the end of the day with fullness_max = 3, if fullness = 1, hp gets -2; if fullness = 2, hp gets -1; if fullness = 3; hp gets +1).
3) If Hp drops to 0, this person would die.
4) Fullness resets to 0 at the start of every new day.

Rob rule:
1) You can take other people's food (steal or rob), the target people should be one of: {', '.join(k for k in self.global_aware if k != self.name)};
...
Each time you take other people's food, your food adds the amount and the the target people, who is being taken food by you, loses the same amount of food.
