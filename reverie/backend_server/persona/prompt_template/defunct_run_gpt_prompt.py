...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  def __func_validate(gpt_response, prompt=""): 
    try: 
      ...
      x = [datetime.datetime.strptime(i.strip(), "%I:%M %p") for i in x.split(" to ")]
...
def run_gpt_prompt_define_to_event(persona, focused_event, sub_only = True, test_input=None, 
                                       verbose=False): 
  def create_prompt_input(init_persona, focused_event, 
                          test_input=None): 
    ...
    parent = ''
    # sub = init_act_desc
    if sub_only and (init_act_desc in init_persona.scratch.f_daily_subtask_parent):
        parent = init_persona.scratch.f_daily_subtask_parent[init_act_desc]
        init_act_desc = init_act_desc[len(parent):].strip().strip('()')  # sub only
      # else:
      #   parent = ''
      #   sub = init_act_desc
      ...
      # init_act_desc = sub
    # if "(" in init_act_desc: 
    # parent = init_act_desc
 
    # if init_act_desc in init_persona.scratch.f_daily_subtask_parent:
    #   init_act_desc = init_persona.scratch.f_daily_subtask_parent[init_act_desc]
    #   parent =  "(".join(init_act_desc.split("(")[:-1])

    # >>>
    first_name, last_name = init_persona.name.split(' ')
    if (first_name not in init_act_desc) and (last_name not in init_act_desc):
      init_act_desc = f"{first_name} {last_name} is {init_act_desc}"

    if parent and (first_name not in parent) and (last_name not in parent):
      parent = f"{first_name} {last_name} is {parent}"
    # first name only
    # if init_persona.name.split(' ')[0] not in init_act_desc:
    #   init_act_desc = f"{init_persona.name.split(' ')[0]} {init_act_desc}"

    # if init_persona.name not in init_act_desc:
    #   init_act_desc = f"{init_persona.name} {init_act_desc}"
    # <<<

    # # >>> context
    # context = ""
    # if focused_event:
    #   for c_node in focused_event["events"]: 
    #     curr_desc = c_node.description.split(" ")
    #     curr_desc[2:3] = ["was"]
    #     curr_desc = " ".join(curr_desc)
    #     context +=  f"{curr_desc}. "
    #   context += "\n"
    #   for c_node in focused_event["thoughts"]: 
    #     context +=  f"{c_node.description}. "
    # # <<<
    ...
    prompt_input += [init_persona.name]
    prompt_input += [
      ', '.join(name for name in init_persona.scratch.global_aware.keys() if not name == init_persona.name)
    ]
    # prompt_input += [target_persona.name]
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # return gpt_response.split("Answer: Option")[-1].strip().lower() 
    # return json.loads(gpt_response.split("Answer:")[-1].strip().lower())
    # return json.loads(gpt_response.split("Answer:")[-1].strip())
    res = robust_load(gpt_response.split("Answer:")[-1].strip())
    ...
    if res['type'] == 'other':
      ...
      assert ('quantity' in res) and ('target' in res)
    return res
  ...
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  gpt_param = {}
  # prompt_template = "persona/prompt_template/v2/define_to_event_v2.txt"
  # prompt_template = "persona/prompt_template/v2/define_to_event_v3.txt"
  prompt_template = "persona/prompt_template/v2/define_to_event_v4.txt"
  prompt_input = create_prompt_input(persona, focused_event,
                                     test_input)
  prompt = generate_prompt(prompt_input, prompt_template)

  fail_safe = get_fail_safe()
  # output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
  #                                  __func_validate, __func_clean_up, verbose=verbose)
...
# if __name__ == '__main__':
  ...
  # sys.path.append('../..')
  # prompt_template = "persona/prompt_template/v2/define_to_event_v2.txt"
  # prompt = generate_prompt(['sleeping', 'February 13, 2023, 13:11:00 PM', 'sleeping'], prompt_template)

  # output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
  #                                  __func_validate, __func_clean_up)
  # run_gpt_prompt_define_to_event(None,
  #                                None,
  #                                test_input=,
  #                                verbose=False)
