...
from utils import HEAL, HP_MAX, FULLNESS_MAX, FOOD_ADD

def is_new_day(self, curr_time):
...
def update_vitality(persona, new_day):
# todo DONE: transfer to scratch(, with save/load/get_str_iss considered)
  if new_day == 'First day':
    # full hp, fullness
    # persona.scratch.hp = HP_MAX  # or something else, died 2days of eating nothing
    # persona.scratch.fullness = 0  # fullness range: [0,3]
    # persona.scratch.food = INIT_FOOD
    # first day alive
    alive = True
  elif new_day:
    # do vital_checks for the previous day

    # >>> checks
    hp = persona.scratch.hp
    if persona.scratch.fullness < FULLNESS_MAX:  # 3 is upper bound
      hp -= (FULLNESS_MAX - persona.scratch.fullness)
      if hp < 0: hp = 0
    else: #elif persona.scratch.fullness >= FULLNESS_MAX:
      hp += HEAL
      if hp > HP_MAX: hp = HP_MAX
    # <<< checks

    would_die = (hp == 0)
    if would_die and utils.eat_failsafe:
      to_fill = FULLNESS_MAX - persona.scratch.fullness
      aux_consumption = to_fill if persona.scratch.food > to_fill else persona.scratch.food
      persona.scratch.aux_consumption += aux_consumption

      persona.scratch.fullness += aux_consumption
      persona.scratch.food -= aux_consumption
    ...
    # anyway:
    persona.scratch.hp = hp    
    
    if persona.scratch.hp == 0:
      # DONE todo: dead
      # DONE todo: add dying reason, and dying message
      # return (persona.scratch.curr_tile, '\ud83d\udc80', f'{persona.scratch.name} is dead')

      alive = False
    else:
      alive = True
      # READY FOR THE NEW DAY
      # todo: per eat, fullness +1 and food -1(, cant eat 0.5 meal), PROMPT NEEDED
      pass
      # DONE todo: give food
      persona.scratch.food += FOOD_ADD
    # reset fullness for NEW DAY
    persona.scratch.fullness = 0  # fullness equals 0
  else:
    # if not new_day, always alive
    alive = True
  return alive
...
class Persona: 
  ...
  def reset_prefix(self, use_proxy=True):
    import utils, openai
    if hasattr(self,'api_prefix') and getattr(self,'api_prefix'):
      utils.api_prefix = ''
    if use_proxy:
      openai.proxy = None

  def set_prefix(self, use_proxy=True):
    import utils, openai
    if hasattr(self,'api_prefix') and getattr(self,'api_prefix'):
      utils.api_prefix = self.api_prefix
    if use_proxy:
  ...
  def move(self, maze, personas, curr_tile, curr_time):
    ...
    #   new_day = "New day"
    # ===
    new_day = is_new_day(self, curr_time)
    # <<<
    # >>>
    # if new_day or not hasattr(self, 'task_calculated'):
    #   self.task_calculated = []
    if new_day or self.scratch.task_calculated is None:
      self.scratch.task_calculated = []
    if new_day or self.scratch.task_activated is None:
      self.scratch.task_activated = []
    # <<<
    ...
    # set prefix, globally
      ...
      # todo: breakpoint-ing to check default value
      # utils.api_prefix = self.api_prefix
    # self.set_prefix()

    # only main part of planning and chat_react applies prefix
    # 1. temporally set at daily_req and gen_decomp
    # 2.  reset the aboved with same condition
    # 3. temporally set at _chat_react if is init_person
    # 4.  reset the aboved with same condition
    # reseted
    plan = self.plan(maze, personas, new_day, retrieved)
    # reset prefix, globally
    # if hasattr(self,'api_prefix') and getattr(self,'api_prefix'):
    #   utils.api_prefix = ''
    ...
    # modification address for person-to-person path finder
    #   once have valid path, dont need to modified memory, as would be perceived automatically, maybe should manually specify poignancy
    #   also can do standard reaction
